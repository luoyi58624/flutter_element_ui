...
class FontUtil {
  ...
  static Future<void> init({
    ...
    await _setElFontWeight();
    if (allowLoadCustomFont) {
      i(fontModel?.fontWeights);
      fontModel != null && fontModel.fontFamily != ''
          ? await FlutterFont.initFont(fontModel)
          : await FlutterFont.initFont(notoSansSc([FontWeight.w500]));
    } else {
      await FlutterFont.initFont(FontPreset.systemFont);
    }
  }

  /// 优化某些设备的[FontWeight]
  static Future<void> _setElFontWeight() async {
    if (kIsWeb) return;
