...
class ElCurrentColor extends InheritedWidget {
  ...
  ///
  /// 注意：这是一个很重要的小部件，有些 Widget 需要将自身颜色跟随背景色，你可能会考虑不设置颜色或者直接设置透明颜色，
  /// 通常情况下这么做没问题，但如果这个小部件的颜色需要参与交互，那么你就必须指定一个颜色（不能是透明，否则动画过程中视觉效果会不统一），
  /// 对于这个需求只需要在父级套上 [ElCurrentColor] 小部件即可，然后传递自身的颜色给后代小部件。
  ///
  /// 提示：在 Element UI 中，所有包含背景颜色的小部件都会注入自身颜色。
